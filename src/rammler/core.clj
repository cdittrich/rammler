...
(defcodec frame-end octet validate-frame-end validate-frame-end)
...
(defcodec field-value (gloss/header octet (comp table-value-types char) not-implemented))
...
;;; Protocol
(defn validate-frame-end [octet]
  (when-not (= 0xCE (bit-and octet 0xFF))
    (throw (ex-info "Invalid frame-end byte encountered" {:frame-end octet})))
  octet)
...
(def amqp-method-codecs
  (into {} (for [[class-id class] amqp-classes [method-id method] (amqp-methods class)]
             [[class-id method-id] (gloss/compile-frame (map second (get-in amqp-method-signatures [class method])))])))

(defcodec amqp-header [(gloss/string :ascii :length 4) octet octet octet octet])

(defn decode-frame-payload [[type channel]]
